                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <div className="flex gap-1">
                      ...
                      <div
                        className="w-2 h-2 rounded-full border border-gray-300"
                        style={{
                          backgroundColor: savedImage.crew.club.secondaryColor,
                        }}
                      />
                    </div>
                  </div>
                )}
                <p>{new Date(savedImage.createdAt).toLocaleDateString()}</p>
              </div>
            </div>
          </div>
        ))}
...
function ImageDetailsModal({
  ...
  const { data: savedImage, isLoading } = trpc.savedImage.getById.useQuery({
    id: imageId,
  })
